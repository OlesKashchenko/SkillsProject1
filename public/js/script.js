...
function initTabs() {
    ...
    $holder.each(function(i, tabber) {
        ...
        $tabs.click(function() {
            ...
            //alexkor 19.08.2015
            if ( !$slides.eq(index).find('.tab_inner').length ) {
                return false;
            }

            var tabOffset = $slides.eq(index).find('.tab_inner').offset().top - $(tabber).offset().top - parseInt($(tabber).css('padding-top'), 10);

            if ($(tabber).data('autoheight') == 1 || $(tabber).data('autoheight') == 'true' || $(tabber).data('autoheight') == true) {
                $(tabber).css({
                    height: tabOffset + Math.max(50, $slides.eq(index).find('.tab_inner').outerHeight())
                });
            }
        });
        $(window).load(function() {
...
/*    // custom select with icons
    ...
    $('.focus_closest').focus(function() {
        $(this).closest('.field').addClass('focus_input');
    });
    $('.focus_closest').hover(function() {
        $(this).closest('.field').addClass('focus_input');
    });
    $('.focus_closest').blur(function() {
        $(this).closest('.field').removeClass('focus_input');
    });
