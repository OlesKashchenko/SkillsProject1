...
@section('main')
    ...
    @if (isset($cardsCategories) && $cardsCategories->count())
        <div class="credit_card_catalog_filter">
            @foreach($cardsCategories as $cardCategory)
                <div class="filter_item" data-id-category="{{$cardCategory->id}}"><span>{{$cardCategory->t('title')}}</span></div>
            @endforeach
            <div class="filter_item act" data-id-category="0"><span>{{__('Все')}}</span></div>
        </div>
    @endif
